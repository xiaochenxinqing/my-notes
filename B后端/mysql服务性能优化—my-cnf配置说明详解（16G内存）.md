...
此配置是 [老男孩](http://www.linuxyw.com/a/wangzhanhuodong/20130608/561.html) 生产线上使用的配置，在 [培训](http://www.linuxyw.com/a/wangzhanhuodong/) 的时候，他给的，我在这里，对各参数添加了中文说明
...
open_[file](http://www.linuxyw.com/a/wenjianguanli/20130505/169.html)s_limit    = 10240
...
#设置每个主机的连接请求异常中断的最大次数，当超过该次数，MYSQL服务器将禁止host的连接请求，直到mysql服务器重启或通过flush hosts [命令](http://www.linuxyw.com/a/Linuxmingling/) 清空此host的相关信息。
...
# 当  [Mysql](http://www.linuxyw.com/a/Mysql/)  访问一个表时，如果该表在缓存中已经被打开，则可以直接访问缓存；如果#还没有被缓存，但是在 Mysql 表缓冲区中还有空间，那么这个表就被打开并放入表缓#冲区；如果表缓存满了，则会按照一定的规则将当前未用的表释放，或者临时扩大表缓存来存放，使用表缓存的好处是可以更快速地访问表中的内容。执行 flush tables 会#清空缓存的内容。一般来说，可以通过查看 [数据库](http://www.linuxyw.com/a/shujuku/) 运行峰值时间的状态值 Open_tables #和 Opened_tables ，判断是否需要增加 table_cache 的值（其中 open_tables 是当#前打开的表的数量， Opened_tables 则是已经打开的表的数量）。即如果open_tables接近table_cache的时候，并且Opened_tables这个值在逐步增加，那就要考虑增加这个#值的大小了。还有就是Table_locks_waited比较高的时候，也需要增加table_cache。

external-locking = FALSE  

#使用–skip-external-locking MySQL选项以避免外部锁定。该选项默认开启

[max_allowed_pack](http://www.linuxyw.com/a/yunweiguzhang/20130520/406.html)et = 32M  
...
#技术导读 [http://blog.webshuo.com/2011/02/16/mysql-sort_buffer_size/](http://blog.webshuo.com/2011/02/16/mysql-sort_buffer_size/)

#dev-doc: [http://dev.mysql.com/doc/refman/5.5/en/server-parameters.html](http://dev.mysql.com/doc/refman/5.5/en/server-parameters.html)
...
# 设置thread_concurrency的值的正确与否, 对mysql的性能影响很大, 在多个 [cp](http://www.linuxyw.com/a/wenjianguanli/20130505/203.html) u(或多核)的情况下，错误设置了thread_concurrency的值, 会导致mysql不能充分利用多cpu(或多核), 出现同一时刻只能一个cpu(或核)在工作的情况。thread_concurrency应设为CPU核数的2倍. 比如有一个双核的CPU, 那么thread_concurrency的应该为4; 2个双核的cpu, thread_concurrency的值应为8
...
log_long_fo[rm](http://www.linuxyw.com/a/wenjianguanli/20130505/195.html)at
...
#这个参数用来设置 InnoDB 存储的数据目录信息和其它内部数据结构的内存池大小，类似于Oracle的lib [rar](http://www.linuxyw.com/a/tiaoyou/20130521/421.html) y cache。这不是一个强制参数，可以被突破。
...
#推荐阅读 [http://www.taobaodba.com/html/221_innodb_max_dirty_pages_pct_checkpoint.html](http://www.taobaodba.com/html/221_innodb_max_dirty_pages_pct_checkpoint.html)
...
#开启该选可以彻底关闭MYSQL的TCP/IP连接方式，如果 [WEB服务](http://www.linuxyw.com/a/WEBfuwu/) 器是以远程连接的方式访问MYSQL数据库服务器的，则不要开启该选项，否则将无法正常连接。
